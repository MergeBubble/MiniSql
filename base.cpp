...
void Table::addData(tuper* t){
    if(t->length()!=attr.num)
        throw TableException("Illegal Tuper Inserted: unequal column size!11");
    int i,j;
    //影响效率的话可以删掉这段
    for(i=0;i<getCsize();i++){
        j = (*t)[i]->flag;
        //std::cout<<j<<std::endl;
        //std::cout<<attr.flag[i]<<std::endl;
        //std::cout<<"     "<<std::endl;
        if(j>attr.flag[i]) {

            //throw TableException("Illegal Tuper Inserted: unequal attributes!22");
        }
        //else if(j<attr.flag[i] && j<=0)
            //throw TableException("Illegal Tuper Inserted: unequal attributes!33");
        }
    T.push_back(t);
}
